...
fn play_station(
    ...
    now_playing_station_icon: &gtk::Image,
    ...
    now_playing_title.set_text(&station.name);
    let br_info = station.bitrate.map_or(String::new(), |b| format!(" ({} kbps)", b));
    now_playing_subtitle.set_text(&format!("{} | {}{}", station.country, station.tags, br_info));
    ...
    if let Ok(pipeline) = playbin_builder.build()
    {
        ...
        let station_icon_fallback = now_playing_station_icon.clone();
                ...
                gstreamer::MessageView::Error(err) => {
                    ...
                    if let Some(idx) = next_index {
                        ...
                        if idx < total {
                            ...
                            let station_icon_rc = station_icon_fallback.clone();
                            let save_mgr_rc = save_mgr_fallback.clone();
                            let prefs_rc = prefs_fallback.clone();
                            
                            glib::idle_add_local_once(move || {
                                play_station(&p_rc, &u_rc, idx, &b_rc, &t_rc, &s_rc, &m_rc, &i_rc, &f_rc, &flag_lbl_rc, &station_icon_rc, &save_mgr_rc, &prefs_rc);
                            });
                        }
                    }
                }
        ...
        if next_index < total {
            ...
            let station_icon_rc = now_playing_station_icon.clone();
            let save_mgr_rc = save_mgr.clone();
            let prefs_rc = prefs.clone();
            glib::idle_add_local_once(move || {
                play_station(&p_rc, &u_rc, next_index, &b_rc, &t_rc, &s_rc, &m_rc, &i_rc, &f_rc, &flag_lbl_rc, &station_icon_rc, &save_mgr_rc, &prefs_rc);
            });
        }
    }
}
            ...
            if let Ok(response) = client.get(&url).header("User-Agent", "RustRadioGTK/1.0").send().await {
                if is_stations {
                    if let Ok(stations) = response.json::<Vec<Station>>().await {
                        // Fetch real DB sync time from the radio-browser.info stats endpoint
                        let sync_time = match client
...
fn build_ui(app: &Application) {
    ...
    let sync_now_btn = Button::builder().label("Update Station List Now").has_frame(false).build();
    let settings_btn = Button::builder().label("Preferences").has_frame(false).build();
    menu_box.append(&sync_now_btn);
    ...
    // Insert search entry right at the top of the main area
    right_content.append(&search_entry);
    ...
    drawing_analyzer.set_draw_func(move |_, cr, width, height| {
        ...
        cr.set_source_rgba(0.20, 0.44, 0.85, 0.95);
        ...
        match current_pref.analyzer_type {
            ...
            AnalyzerType::BlocksWithHold => {
                ...
                for (i, &val) in bars.iter().enumerate() {
                    ...
                    cr.set_source_rgba(0.20, 0.44, 0.85, 0.40);
    ...
    let now_playing_left = Box::builder()
        ...
        .margin_end(8)
        .build();
    let now_playing_station_icon = gtk::Image::from_icon_name("audio-x-generic-symbolic");
    now_playing_station_icon.set_icon_size(gtk::IconSize::Large);
    now_playing_station_icon.set_valign(gtk::Align::Center);
    now_playing_left.append(&now_playing_station_icon);
    ...
    let now_playing_text_box = Box::builder()
        ...
        .valign(gtk::Align::Center)
        .build();
    // Title row: station name + elapsed time side by side
    ...
    let prefs_sync = app_prefs.clone();
    let ui_sync = ui_state.clone();
    let popover_menu_closer = menu_popover.clone();
    let sync_label = last_updated_label.clone();
    sync_now_btn.connect_clicked(move |_| {
        popover_menu_closer.popdown();
        sync_label.set_text("Syncing...");
        let mut ui = ui_sync.lock().unwrap();
        ...
        drop(ui);
        fetch_data_async(url, is_stations, view, prefs_sync.clone(), tx_sync.clone());
    });
    ...
    fav_menu_btn.connect_clicked(move |_| {
        ...
        for station in &ui.favorites {
            ...
            let row = ActionRow::builder()
                ...
                .subtitle(format!("{} | {}{}", station.country, station.tags, br_badge))
                .activatable(true)
                .build();
        ...
        if view.playlists.is_empty() {
            ...
            for (i, s) in stations.iter().enumerate() {
                ...
                rm_btn.connect_clicked(move |_| {
                    ...
                    if i < v.playlists.len() {
                        v.playlists.remove(i);
    ...
    let station_icon_row_click = now_playing_station_icon.clone();
    ...
    list_box.connect_row_activated(move |_, row| {
        ...
        match mode {
            ViewType::Stations | ViewType::Favorites | ViewType::Recent | ViewType::Playlists | ViewType::CustomTag(_) => {
                drop(ui);
                play_station(&state_row_click, &ui_row_click, index, &ui_play_row_click, &title_row_click, &sub_row_click, &meta_row_click, &info_row_click, &fav_toggle_click, &flag_lbl_row_click, &station_icon_row_click, &save_mgr_row_click, &prefs_play_row_click);
            }
            ViewType::Tags => {
    ...
    let station_icon_ctrl = now_playing_station_icon.clone();
    ...
    play_pause_btn.connect_clicked(move |btn| {
        let mut player = state_play_pause.lock().unwrap();
        if let Some(pipeline) = &player.pipeline {
            ...
            drop(player);
            play_station(&state_play_pause, &ui_play_pause, 0, &btn_play_pause_clone, &np_title_ctrl, &np_sub_ctrl, &meta_lbl_ctrl, &info_lbl_ctrl, &fav_toggle_ctrl, &flag_lbl_ctrl, &station_icon_ctrl, &save_mgr_play_pause, &prefs_play_pause);
        }
    });
    ...
    let station_icon_prev = now_playing_station_icon.clone();
    let save_mgr_prev = app_state_manager.clone();
    let prefs_prev = app_prefs.clone();
    prev_btn.connect_clicked(move |_| {
        let current_idx = ui_prev.lock().unwrap().current_index;
        if let Some(idx) = current_idx {
            if idx > 0 {
                let new_idx = idx - 1;
                play_station(&state_prev, &ui_prev, new_idx, &btn_prev_ui, &np_title_prev, &np_sub_prev, &meta_lbl_prev, &info_lbl_prev, &fav_toggle_prev, &flag_lbl_prev, &station_icon_prev, &save_mgr_prev, &prefs_prev);
    ...
    let station_icon_next = now_playing_station_icon.clone();
    let save_mgr_next = app_state_manager.clone();
    let prefs_next = app_prefs.clone();
    next_btn.connect_clicked(move |_| {
        let (current_idx, total) = {
            let ui = ui_next.lock().unwrap();
            let count = match ui.current_view {
                ViewType::Favorites => ui.favorites.len(),
                ViewType::Recent => ui.recent_stations.len(),
                ViewType::Playlists => ui.playlists.len(),
                _ => ui.stations.len(),
            };
            (ui.current_index, count)
        };
        if let Some(idx) = current_idx {
            if idx + 1 < total {
                let new_idx = idx + 1;
                play_station(&state_next, &ui_next, new_idx, &btn_next_ui, &np_title_next, &np_sub_next, &meta_lbl_next, &info_lbl_next, &fav_toggle_next, &flag_lbl_next, &station_icon_next, &save_mgr_next, &prefs_next);
        ...
        while let Ok(data) = rx.recv().await {
            ...
            match data {
                ...
                IncomingData::Stations(stations_list, server_sync_time) => {
                    ...
                    if let Some(text) = browsing_text {
                        now_browsing_label_clone.set_text(&text);
                    }
                    drop(ui);
                    state_rx_save.save_data();

                    stations_count_label_clone.set_text(&format!("{} stations displayed", stations_list.len()));

                    for station in stations_list {
                        let title_text = station.name.trim().to_string();
                        let row = ActionRow::builder()
                            .title(title_text)
                            .subtitle(format!("{} | {}", station.country, station.tags))
                            .activatable(true)
                            .build();
                        ...
                        if !flag_str.is_empty() {
                            let flag_lbl = Label::builder()
                                .label(&flag_str)
                                .valign(gtk::Align::Center)
                                .build();
                            row.add_suffix(&flag_lbl);
                        }
                        if let Some(br) = station.bitrate {
                            if br > 0 {
                                let br_lbl = Label::builder()
                                    .label(&format!("{} kbps", br))
                                    .css_classes(vec!["caption".to_string(), "dim-label".to_string()])
                                    .valign(gtk::Align::Center)
                                    .build();
                                row.add_suffix(&br_lbl);
                            }
                        }
                        let is_fav = ui_rx.lock().unwrap().favorites.iter().any(|f| f.stationuuid == station.stationuuid);
                        let fav_icon = if is_fav { "starred-symbolic" } else { "non-starred-symbolic" };
                        let row_fav_btn = Button::builder()
                            .icon_name(fav_icon)
                            .css_classes(vec!["flat".to_string()])
                            .valign(gtk::Align::Center)
                            .build();
                        let station_for_fav = station.clone();
                        let ui_for_row_fav = ui_rx.clone();
                        let save_mgr_row_fav = state_rx_save.clone();
                        row_fav_btn.connect_clicked(move |btn| {
                            let mut ui = ui_for_row_fav.lock().unwrap();
                            if let Some(pos) = ui.favorites.iter().position(|f| f.stationuuid == station_for_fav.stationuuid) {
                                ui.favorites.remove(pos);
                                btn.set_icon_name("non-starred-symbolic");
                            } else {
                                ui.favorites.push(station_for_fav.clone());
                                btn.set_icon_name("starred-symbolic");
                            }
                            drop(ui);
